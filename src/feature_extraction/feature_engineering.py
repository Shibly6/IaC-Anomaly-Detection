...
# Setup logging - FIX: Use consistent logger name
logger = logging.getLogger("anomaly_detector.feature_engineering")
...
def select_discriminative_features(df, n_features=15):
    """
    ...
    priority_features = [
        ...
        'security_penalty',  # Security penalty for public access
        'has_critical_gaps'  # Has critical security gaps
    ]
    ...
    if len(available_priority) < n_features:
        ...
        if other_features and len(df) > 1:
            try:
                ...
                logger.warning(f"Could not calculate feature variances: {str(e)}")
    ...
    logger.info(f"Selected {len(selected_features)} discriminative features: {selected_features}")
...
def engineer_features_pipeline(df):
    ...
    try:
        ...
        logger.info(f"After numerical features: {df.shape[1]} columns")

        # Step 2: Create security score features
        df = create_security_score_features(df)
        logger.info(f"After security score features: {df.shape[1]} columns")

        # Step 3: Create interaction features
        df = create_interaction_features(df)
        logger.info(f"After interaction features: {df.shape[1]} columns")

        # Step 4: Create anomaly indicator features
        df = create_anomaly_indicator_features(df)
        logger.info(f"After anomaly indicator features: {df.shape[1]} columns")
        ...
        discriminative_features = select_discriminative_features(df, n_features=15)
